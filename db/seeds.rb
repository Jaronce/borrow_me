# file = File.open("#{Rails.root}/db/address.txt")
...
random_name = Faker::FunnyName.unique.name
...
random_user = users[rand(11)]


p "Adding Male and Female products"

products = []
@women_photos.each do |photo|
  products << Product.create!(user: random_user, name: Faker::Name.unique.female_first_name, price: rand(100)*10, city: @montreal.sample(1), gender: "female", remote_photo_url: photo)
end
@men_photos.each do |photo|
  products << Product.create!(user: random_user, name: Faker::Name.unique.male_first_name, price: rand(100)*10, city: @montreal.sample(1), gender: "male", remote_photo_url: photo)
end
...
products.each do |product|
  ...
  product_skill.skill = skills[rand(8)]
  product_skill.save!
end
